...
function M.create_new_note()
  ...
  local filepath = constants.NOTES_DIR .. filename

  -- Create directory if it doesn't exist
  vim.fn.mkdir(constants.NOTES_DIR, 'p')

  -- Open the file
  vim.cmd('edit ' .. vim.fn.fnameescape(filepath))

  -- Change to git repository root (use hardcoded path for reliability)
  vim.cmd('cd ' .. vim.fn.fnameescape(constants.LIFE_REPO_ROOT))
...
function M.create_daily_note()
  ...
  local daily_dir = constants.get_daily_path(year)
